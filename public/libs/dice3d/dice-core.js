...
diceCore.element = function (name, props, place, content) {
  ...
  if (props)
    for (const i in props) dom.setAttribute(i, props[i])
...
diceCore.clas = function (sel, oldclass, newclass) {
  const oc = oldclass ? oldclass.split(/\s+/) : [],
    nc = newclass ? newclass.split(/\s+/) : []
  let classes = (sel.getAttribute("class") || "").split(/\s+/)
    ...
    promise: function () {
      return {
        then: function (callback) {
          const deferred = diceCore.deferred(),
